...
class Lang
{
    ...
    /**
     *
     * @var \MGModule\ResellersCenter\Lang
     */
     ...
     * @param type $dir
     * @param type $lang
     * @return \MGModule\ResellersCenter\Lang
     */
...
//            self::$instance->fillLangFile = process\MainInstance::I()->isDebug();
    ...
    public static function getLang()
    {
        //TODO
        $language = '';
        if (isset($_SESSION['Language']) && !defined('ADMINAREA')) // GET LANG FROM SESSION
        {
        ...
        {
            $row = MySQL\Query::query("SELECT `language` FROM tbladmins WHERE `id` = " . $_SESSION['adminid'] . " LIMIT 1")->fetch();
            if( $row['language'] )
            {
                $language = $row['language'];
            }
    ...
    {
        $files = [
            self::getInstance()->dir . DS . $lang . '.php',
            self::getInstance()->dir . DS . 'overrides'. DS. $lang . '.php',
        ];
