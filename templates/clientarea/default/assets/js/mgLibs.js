var JSONParser = {
    ...
    request: function (action, data, callback, loader, disableErrors) {
        ...
        if(action.indexOf("|") > 0) {
            ...
            if(JSONParser.getParameterByName("mg-page")){
                page = JSONParser.getParameterByName("mg-page");
            }
        }
        
        if(page) 
        {
            if(this.url.indexOf("mg-page") > 0) {
                this.url = this.url.substr(0, this.url.indexOf("mg-page")-1);
            }
            
            this.url += "&mg-page="+page;
        }
...
jQuery.fn.MGModalActions = function(){    
                ...
                this.on('click','*[data-modal-id]', function(event){
                    ...
                    jQuery('#'+modal).find('[data-modal-title]').text(title ? title : target);
...
/* Twenty-One template need this selector to work as in Six template.
*  The use of class 'active-client' distinguishes template Six from TwentyOne */
var RC_TwentyOne_Helper = {
    twentyOneLiSelector: function() {
